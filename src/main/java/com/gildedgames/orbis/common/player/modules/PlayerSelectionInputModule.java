...
public class PlayerSelectionInputModule extends PlayerOrbisModule
{
	...
	public PlayerSelectionInputModule(final PlayerOrbis playerOrbis)
	{
		super(playerOrbis);

		this.non_uniform = new SelectionInputDragged(this.getEntity(), "Non-Uniform",
				OrbisCore.getResource("godmode/selection_mode_icons/non_uniform.png")).createFromCenter(false).uniform(false);
		this.uniform = new SelectionInputDragged(this.getEntity(), "Uniform",
				OrbisCore.getResource("godmode/selection_mode_icons/uniform.png")).createFromCenter(false).uniform(true);
		this.centered = new SelectionInputDragged(this.getEntity(), "Centered",
				OrbisCore.getResource("godmode/selection_mode_icons/centered.png")).createFromCenter(true).uniform(true);
		this.brush = new SelectionInputBrush(playerOrbis, playerOrbis.getWorld());
	...
	public void setCurrentSelectionInput(final int index)
	{
		...
		{
			OrbisCore.network().sendPacketToServer(new PacketChangeSelectionInput(this.currentSelectionTypeIndex));
		}
	}
	...
	{
		IShapeSelector selector = this.getPlayer().powers().getCurrentPower().getShapeSelector();
		final ItemStack held = this.getPlayer().powers().getEntity().getHeldItemMainhand();
