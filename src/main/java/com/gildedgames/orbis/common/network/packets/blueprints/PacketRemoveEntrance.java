			...
			try
			{
				...
				if (data instanceof BlueprintData)
				{
					...
					if (player.world.getMinecraftServer().isDedicatedServer())
					{
						if (message.id == null)
						{
							OrbisAPI.network().sendPacketToAllPlayers(new PacketRemoveEntrance(message.worldObjectId, message.entranceId));
						}
						else
						{
							OrbisAPI.network().sendPacketToAllPlayers(new PacketRemoveEntrance(message.id, message.entranceId));
						}
					}
				}
			}
