...
public class NavigatorNodeBlueprintStacker implements INavigatorNode
{
	private static final ResourceLocation TEXTURE = OrbisCore.getResource("navigator/blueprint_file.png");

	private final File file;

	private final GuiTexture icon = new GuiTexture(Dim2D.build().area(23).flush(), TEXTURE);

	public NavigatorNodeBlueprintStacker(final File file)
	{
		if (file.isDirectory())
		{
			throw new RuntimeException("File given to NavigatorNodeBlueprintSTacker is a directory! Aborting.");
		}
