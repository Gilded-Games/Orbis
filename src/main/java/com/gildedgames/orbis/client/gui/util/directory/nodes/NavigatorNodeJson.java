...
public class NavigatorNodeJson implements INavigatorNode
{
	private static final ResourceLocation TEXTURE = OrbisCore.getResource("navigator/json_file.png");

	private final File file;

	private final GuiTexture icon = new GuiTexture(Dim2D.build().area(23).flush(), TEXTURE);

	public NavigatorNodeJson(final File file)
	{
		if (file.isDirectory())
		{
			throw new RuntimeException("File given to NavigatorNodeJson is a directory! Aborting.");
		}
